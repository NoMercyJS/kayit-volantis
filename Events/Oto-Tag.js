...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  const embed = new discord.MessageEmbed().setAuthor(user.displayName, user.user.avatarURL({dynamic: true})).setFooter("No Mercy ♥ Volantis").setColor(client.randomColor()).setTimestamp();
  ...
  if(newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(ayarlar.ekipRolu)){
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi ismine Tagımızı (\`${ayarlar.tag}\`)  alarak <@&${ayarlar.ekipRolu}> ekibimize katıldı!`).setColor("#32FF00")).catch();
